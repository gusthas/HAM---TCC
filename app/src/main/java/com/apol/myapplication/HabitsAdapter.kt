...
class HabitsAdapter(
    ...
    override fun onBindViewHolder(holder: HabitViewHolder, position: Int) {
        ...
        if (emoji.isNotEmpty() && context is habitos) {
            ...
            holder.icone.setImageResource(R.drawable.ic_habits) // Ícone padrão
        }

        // Lógica de Dias Seguidos e Mensagem
        holder.streakDays.text = "${habit.streakDays} dias seguidos"
        holder.message.text = habit.message

        // Lógica Visual do Check
        if (habit.count > 0) {
